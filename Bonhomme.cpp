...
char keypressed;        // Le caractère lu au clavier dans la gestion des entrées clavier
...
void keyboard(unsigned char touche, int x, int y);  // Gère les touches classiques du clavier
    ...
    switch(touche) {
        ...
        case 'T':
            if (isBottomConnected(2)) {                                 // si la tête est connectée au tronc
                if (boule[2][1][1] < sFactorMax) {                      //--- et si le facteur max n'est pas atteint
                ...
                else {                                                  //--- sinon (le facteur max est atteint)
        ...
        case 't':
            if (isBottomConnected(2)) {                           // si la tête est connectée au tronc
                ...
                else {                                            //--- sinon (la distance est insuffisante pour translater)
